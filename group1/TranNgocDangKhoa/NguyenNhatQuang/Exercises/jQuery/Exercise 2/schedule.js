...
function displaySchedule() {
    ...
    if(track1Checkbox.prop('checked') && track2Checkbox.prop('checked')){
        ...
        if(track1Checkbox.prop('checked') && !track2Checkbox.prop('checked')){
            select =1;
        }else if(!track1Checkbox.prop('checked') && track2Checkbox.prop('checked')){
            select =2;
        }
        for (var i = 0; i < schedule.length; i++) {
            if(schedule[i]['tracks'].indexOf(select)!==-1){
                var li = createSessionElement(schedule[i]);
                list.appendChild(li);
            }
        }
