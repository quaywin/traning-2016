...
track1Checkbox.bind("click", displaySchedule);
track2Checkbox.bind("click", displaySchedule);
...
function displaySchedule() {
    ...
    for (var i = 0; i < schedule.length; i++) {
        kt = false;
        if (track1Checkbox.is(":checked"))
            if (schedule[i]["tracks"].indexOf(1) !== -1)
                kt = true;
        if (track2Checkbox.is(":checked"))
            if (schedule[i]["tracks"].indexOf(2) !== -1)
                kt = true;
        if (kt) {
